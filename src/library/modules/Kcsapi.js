...
(function(){
	...
	window.Kcsapi = {
		...
		"api_req_mission/result":function(params, response, headers){
			...
			if(response.api_data.api_clear_result > 0){
					KC3Quest.incrementBatch(4, { expedId: Number(expedNum) });
					KC3Network.trigger("Quests");
			}
		...
		"api_req_kousyou/destroyitem2":function(params, response, headers){
			...
			$.each(itemIds.split("%2C"), function(index, itemId){
				...
				if(!gearMaster){
					...
					KC3Quest.incrementBatch(6, { gearMaster });
				}
		...
		"api_get_member/mapinfo":function(params, response, headers){
			...
			if(ConfigManager.alert_pre_sortie > 0) {
				...
				if (fleetNo <= 2 && PlayerManager.fleets[2].isStrikingForce()) fleetNo = 3;
	...
	function resultScreenQuestFulfillment(data,isPvP){
		...
		if(!isPvP) {
			[ /* Rank Requirement Table */
			  /* Define: [Quest ID, index of tracking, [world, map], isBoss, isCheckCompos, [bossEdges]] */
				...
				[ /* A RANK */
					...
					[973,0,[3,1], true, true], // By11: 1st requirement: [W3-1] A-rank+ the boss node
				...
				[ /* S RANK */
					...
					[975,0,[1,5], true, true], // By12: 1st requirement: [W1-5] S-rank the boss node
			...
			KC3Quest.incrementBatch(3, { rankPt });
		}
	}
